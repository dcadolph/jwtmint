...
type CheckFunc func(ctx context.Context, c jwt.MapClaims) error

// Check calls the receiver, implementing the CheckFunc interface for adapter use.
func (f CheckFunc) Check(ctx context.Context, c jwt.MapClaims) error { return f(ctx, c) }
...
func Chain(checks ...CheckFunc) CheckFunc {
	return func(ctx context.Context, c jwt.MapClaims) error {
...
//
// leeway tolerates small clock skew between the issuer and verifier; pass 0 to disable.
func CheckNotExpired(leeway time.Duration) CheckFunc {
	return func(_ context.Context, c jwt.MapClaims) error {
		exp, err := ExpiresAt(c)
		if err != nil {
			return err
		}
		if exp.Add(leeway).Before(time.Now()) {
...
// CheckNotBeforeReady returns a CheckFunc that errors if the "nbf" claim is malformed,
// or if it is in the future beyond the given leeway.
//
// Missing "nbf" is treated as ready (no error). Malformed "nbf" (wrong type, bad
// timestamp) is treated as failure — silently swallowing it would let bad tokens pass.
//
// leeway tolerates small clock skew; pass 0 to disable.
func CheckNotBeforeReady(leeway time.Duration) CheckFunc {
	return func(_ context.Context, c jwt.MapClaims) error {
		nbf, err := NotBefore(c)
		if err != nil {
			...
			return fmt.Errorf("%w: malformed nbf: %w", pkgerr.ErrInvalidClaims, err)
		}
		if nbf.After(time.Now().Add(leeway)) {
